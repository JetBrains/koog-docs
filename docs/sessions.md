...
This guide provides detailed information about LLM sessions in the Code Engine platform, including how to work with read and write sessions, manage conversation history, and make requests to language models.

## Table of Contents

1. [Introduction](#introduction)
2. [Understanding LLM Sessions](#understanding-llm-sessions)
   - [What are LLM Sessions?](#what-are-llm-sessions)
   - [Read vs. Write Sessions](#read-vs-write-sessions)
   - [Session Lifecycle](#session-lifecycle)
3. [Working with LLM Sessions](#working-with-llm-sessions)
   - [Creating Sessions](#creating-sessions)
   - [Session Scope and Thread Safety](#session-scope-and-thread-safety)
   - [Accessing Session Properties](#accessing-session-properties)
4. [Making LLM Requests](#making-llm-requests)
   - [Basic Request Methods](#basic-request-methods)
   - [When Requests Actually Happen](#when-requests-actually-happen)
   - [Request Methods with Tools](#request-methods-with-tools)
   - [Structured and Streaming Requests](#structured-and-streaming-requests)
5. [Managing Conversation History](#managing-conversation-history)
   - [Updating the Prompt](#updating-the-prompt)
   - [How Responses Update History](#how-responses-update-history)
   - [History Compression](#history-compression)
6. [Tool Execution in Sessions](#tool-execution-in-sessions)
...
LLM sessions are a fundamental concept in the Code Engine platform that provide a structured way to interact with language models (LLMs). They manage the conversation history, handle requests to the LLM, and provide a consistent interface for executing tools and processing responses.

Understanding how LLM sessions work is crucial for creating effective agent workflows that maintain context, manage history efficiently, and interact with LLMs in a controlled manner.
...
1. **Write Sessions (`LocalAgentLLMWriteSession`)**: Allow modifying the prompt and tools, making LLM requests, and executing tools. Changes made in a write session are persisted back to the LLM context.

2. **Read Sessions (`LocalAgentLLMReadSession`)**: Provide read-only access to the prompt and tools. They are useful for inspecting the current state without making changes.
...
These functions take a lambda block that executes within the context of the session. The session is automatically closed when the block completes.
...
2. **`requestLLMWithoutTools()`**: Makes a request to the LLM with the current prompt but without any tools, returning a single response.

3. **`requestLLMMultiple()`**: Makes a request to the LLM with the current prompt and tools, returning multiple responses.
...
1. **Explicit Invocation**: Requests only happen when you call methods like `requestLLM()`, `requestLLMWithoutTools()`, etc.

2. **Immediate Execution**: When you call a request method, the request is made immediately, and the method blocks until a response is received.
...
In practice, you typically don't need to check the response type manually, as the agent graph handles this routing automatically.
...
2. **Keep Sessions Short**: Sessions should be focused on a specific task and closed as soon as possible to release resources.
...
5. **Prefer High-Level Abstractions**: When possible, use the node-based API (e.g., `nodeLLMSendStageInput`, `nodeLLMRequest`) instead of directly working with sessions.

6. **Be Mindful of Thread Safety**: Remember that write sessions block other sessions, so keep write operations as short as possible.

7. **Use Structured Requests for Complex Data**: When you need the LLM to return structured data, use `requestLLMStructured` instead of parsing free-form text.

8. **Use Streaming for Long Responses**: For long responses, use `requestLLMStreaming` to process the response as it arrives.

## Troubleshooting

### Session Already Closed

If you see an error like "Cannot use session after it was closed", you're trying to use a session after its lambda block has completed. Make sure all session operations are performed within the session block.
